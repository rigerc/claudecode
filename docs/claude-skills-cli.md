...
Tell Claude Code to create skills with cli tool.

Using bash/shell:

```
"Use claude-skills-cli to create a skill for my API client code"
```
...
```bash
npx claude-skills-cli add-hook            # Global (all projects)
npx claude-skills-cli add-hook --project  # Project (committed)
npx claude-skills-cli add-hook --local    # Project-local (gitignored)
```
...
```bash
npx claude-skills-cli add-hook  # Recommended: global (all projects)
```
