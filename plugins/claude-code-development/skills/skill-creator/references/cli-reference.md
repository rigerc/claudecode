...
```
✅ Skill created at: <location>/my-skill

Next steps:
1. Edit <location>/my-skill/SKILL.md with your skill instructions
2. Add detailed documentation to references/
3. Add executable scripts to scripts/
4. Remove example files you don't need

Validate with: claude-skills-cli validate <location>/my-skill
```

Where `<location>` depends on the chosen save location.
...
**Note**: Packaging is only needed when sharing skills with others or
uploading to Claude.ai. Local skills (project, user, or plugin) work
without packaging.
...
- **Do package** if sharing with others or uploading to Claude.ai
- **Don't package** for local development (project/user/plugin skills work
...
```json
{
	"scripts": {
		...
		"skill:validate:user": "claude-skills-cli validate ~/.claude/skills/*",
...
For plugin projects, you can also add:

```json
{
	"scripts": {
		"skill:validate:plugin": "claude-skills-cli validate ./plugins/*/skills/*"
	}
}
```
