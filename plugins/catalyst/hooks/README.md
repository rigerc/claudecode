...
**Step 1: Install Rust hooks (recommended - one time setup):**
```bash
...
**Step 2: Copy wrapper to your project:**
```bash
...
**Performance:** ~2ms startup time (imperceptible)
...
All hooks are implemented in Rust for:
- **Maximum performance** (~2ms startup vs 100-200ms for interpreted languages)
- **Zero runtime dependencies** (single binary)
- **Minimal memory** (3-5MB vs 30-50MB)
- **Safety guarantees** (no null pointers, no data races)

**See:**
- [RustHooks/README.md](./RustHooks/README.md) - Full Rust implementation guide
- [PERFORMANCE_COMPARISON.md](./PERFORMANCE_COMPARISON.md) - Performance analysis
- [RustHooks/DATABASES.md](./RustHooks/DATABASES.md) - SQLite-backed state management
...
## Quick Start: Two Essential Hooks Only

Most projects only need these two hooks:

```bash
# 1. Install Rust hooks once
cd .claude/hooks/RustHooks
./install.sh

# 2. In your project, create wrappers
cd your-project/.claude/hooks/
...
# 3. Update settings.json (see examples above)
```

**That's it!** Skills will now auto-activate.
...
3. **Recommend standalone Rust installation** for best performance
4. **Ask before adding Stop hooks** - they can block if misconfigured
